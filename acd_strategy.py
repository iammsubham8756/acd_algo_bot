...
BINANCE_API_KEY = "mhDCWMdClVucdu52sxHTwHUYhUn0srSLHmIMsgdiwsezUBYHzmOoIfkCTX3Hmlbk"
BINANCE_API_SECRET = "PL2u7QeImZKZejLPohjUBNWQiqMjYWNegqkry6xaFiSJS91TLIGT2Aa3kFEaj9OV"

def send_telegram(message):
    if not TELEGRAM_TOKEN or not CHAT_ID:
        print("Telegram not configured.")
        return
    url = f"https://api.telegram.org/bot8396714563:AAFL_3DGaVNRwbhByqEQ2cUpjEWBfot3ous/sendMessage"
    data = {"chart_id": CHAT_ID, "text":
            message}
    from binance import Client 
    client = Client(BINANCE_API_KEY, BINANCE_API_SECRET)
